    ...
    public class dwnldRegFormService : IdwnldRegFormService
    {
        ...
        public async Task<List<Student_Mst>> GetStudentDetails(string Collegeid, string CollegeCode, string StudentName, string FacultyId)
        {
            ...
            try
            {
                ...
                var result = await _context.StudentDetails.FromSqlRaw("EXEC sp_GetStudentDetails @CollegeId, @CollegeCode, @StudentName, @FacultyId,@SubCategory", CollegeIdPr, CollegCodePr, StudentNamePr, FacultyIdPr, SubCategory).ToListAsync();
